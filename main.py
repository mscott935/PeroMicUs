...
class Ui_MainWindow(object):
    ...
    def analyze(self):
        ...
        opts = {
            'input_dir': self.lineEdit_inputDir.text(),
        ...
        try:
            ...
            error_dialog.exec_()
            return
        except exceptions.FilenameParseError as error: # Duplicate errors, how do we neaten this?
            error_dialog = QtWidgets.QErrorMessage()
            error_dialog.setWindowModality(QtCore.Qt.WindowModal)
            error_dialog.showMessage(error.error_string())
            error_dialog.exec_()
            return
        except exceptions.NoInputError as error: # Duplicate errors, how do we neaten this?
            error_dialog = QtWidgets.QErrorMessage()
            error_dialog.setWindowModality(QtCore.Qt.WindowModal)
            error_dialog.showMessage(error.error_string())
            error_dialog.exec_()
    ...
    def updateSpectrogram(self):
        ...
        self.label_specInfo_confidence.setText(f"Classification confidence: {currentVoc['svm_confidence']}")
    ...
    def updateSpectrogramClassification(self, boxIndex):
        ...
        if boxIndex != 0:
            try:
                ...
                self.label_specInfo_confidence.setText(f"Classification confidence: ")
