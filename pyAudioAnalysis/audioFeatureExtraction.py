...
def stSpectogram(signal, fs, win, step, filename, PLOT=False):
    """
    Short-term FFT mag for spectogram estimation:
    Returns:
        a numpy array (nFFT x numOfShortTermWindows)
...
def stFeatureExtraction(signal, fs, win, step):
    ...
    n_time_spectral_feats = 11
    n_harmonic_feats = 0
    n_mfcc_feats = 13
    # n_chroma_feats = 13
    n_chroma_feats = 0 # chroma features are not applicable to our data
...
#    n_total_feats = n_time_spectral_feats + n_mfcc_feats + n_harmonic_feats
    ...
    feature_names.append("spectral_rolloff_90")
    feature_names.append("spectral_rolloff_50")
    feature_names.append("spectral_rolloff_25")
    feature_names.append("spectral_rolloff_10")
    ...
    while (cur_p + win - 1 < N):                        # for each short-term window until the end of signal
        ...
        curFV[7] = stSpectralRollOff(X, .90, fs)        # spectral rolloff
        curFV[8] = stSpectralRollOff(X, .50, fs)        # spectral rolloff
        curFV[9] = stSpectralRollOff(X, .25, fs)        # spectral rolloff
        curFV[10] = stSpectralRollOff(X, .10, fs)        # spectral rolloff
